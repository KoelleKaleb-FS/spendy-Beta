...
export default function AuthTest() {
  ...
  async function runTest() {
    ...
    try {
      ...
      const [forecastRes, recurringRes] = await Promise.all([
        fetch(`${API_BASE}/api/budget/forecast`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${API_BASE}/api/recurring/monthly-impact`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);

      const forecastJson = await forecastRes
        .json()
        .catch(() => ({ status: forecastRes.status }));
      const recurringJson = await recurringRes
        .json()
        .catch(() => ({ status: recurringRes.status }));

      console.log("/api/budget/forecast", forecastRes.status, forecastJson);
      console.log(
        "/api/recurring/monthly-impact",
        recurringRes.status,
        recurringJson
      );
  ...
  return (
    ...
    >
      <h4>Auth SDK Test</h4>
      <p>
        Checks getAccessTokenSilently() then calls forecast + recurring
        endpoints.
      </p>
